...
# 1. 数据加载和预处理
df = pd.read_csv('Thunderbolts_comments.csv')
...
# 4. 构建LSTM模型
model = Sequential()
model.add(Embedding(input_dim=5000, output_dim=512, input_length=150))
...
plt.tight_layout()
plt.savefig('th_training_metrics.png')
plt.show()
...
df.to_csv('th_classified_comments.csv', index=False)
...
print("\n分类完成，结果已保存到th_classified_comments.csv")
